...
class ListaSplit extends JFrame implements ListSelectionListener {
    ...
    private final int alturaJanela = larguraJanela * 3/2;


    private ListaSplit() throws SQLException, IOException {
        super("Astronaut Database"); // ajusta titulo

        Image icon = new ImageIcon("imagens/vetor/astronaut.ico").getImage();
        setIconImage(icon);
    ...
    public void valueChanged(ListSelectionEvent e) {
        ...
        String sb = "ID: \t" + astronautas.get(i).getIdAstronauta() + "\n" + "NOME: \t" + astronautas.get(i).getPrimeiro_Nome() + " " + astronautas.get(i).getNome_do_Meio() + " " + astronautas.get(i).getSobrenome() + "\n" +
                "PAIS: \t" + mostraPais(astronautas.get(i).getPais_Nasc()) + "\n" +
                "ESTADO: \t" + astronautas.get(i).getEstado_Nasc() + "\n" +
                "CIDADE: \t" + astronautas.get(i).getCidade_Nasc() + "\n" +
                "NASC: \t" + sdf.format(astronautas.get(i).getDtNasc()) + "\n" +
                "SEXO: \t" + (astronautas.get(i).getSexo().charAt(0) == 'M' ? "masculino" : "feminino") + "\n\n";
    ...
    private String mostraPais(String paisISO){
        for (Pais p : paises){
